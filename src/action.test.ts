...
test("packaged prompts load independently of the working directory", async () => {
  await inTemporaryDirectory(() => {
    ...
    assert.match(readPrompt("verify.md"), /already raised in two earlier rounds/);
...
test("review action runs finders at high reasoning and the verifier at medium", () => {
...
test("review verdict fails closed and resolves only current PR threads", async () => {
  ...
  await inTemporaryDirectory(async (directory) => {
    ...
    fs.writeFileSync(
      path.join(directory, ".codex/raised.json"),
      JSON.stringify({
...
test("re-reviews defer findings on unchanged code and on dropped precedent", () => {
  ...
  assert.deepEqual(
    kept.map((entry) => `${entry.severity} ${entry.title}`),
    [
      "MEDIUM Fresh medium",
  ...
  assert.deepEqual(
    conceded.map(({ id, reply }) => ({ id, invariant: reply?.includes("Page backward until the origin is found.") })),
    [{ id: "t2", invariant: true }],
  );
