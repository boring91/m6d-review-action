...
The action rejects drafts, forked pull requests, closed pull requests, and pull requests targeting a branch other than `base-branch`. Only owners, members, and collaborators can trigger `@review` commands or review-reply evaluations. Review and reply runs use `gpt-5.6-sol` with `xhigh` reasoning effort. Codex runs with `danger-full-access` and an approval policy of `never`.
