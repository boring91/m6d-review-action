...
The action rejects drafts, forked pull requests, closed pull requests, and pull requests targeting a branch other than `base-branch`. Only owners, members, and collaborators can trigger `@review` commands or review-reply evaluations. A command comment must consist of exactly `@review`; mentions inside longer text are ignored. Review and reply runs use `gpt-5.6-sol` with `high` reasoning effort. Codex runs with `danger-full-access` and an approval policy of `never`.
