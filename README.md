...
Re-reviews are incremental. After the first round, the action diffs the commit of the last approve-or-request-changes review against the new head (thread replies posted by reply mode do not count) and only that changed code is in scope for new `MEDIUM` and `LOW` findings; `HIGH` and `CRITICAL` may still be raised anywhere. Titles from every earlier "Considered and dropped" list are precedent (except duplicates merged into a confirmed finding), and a finding matching one is not re-raised unless its code changed since. Findings deferred by either rule are appended to the dropped list so they stay visible without adding a round. A force push or a compare over 300 files falls back to full scope. A thorough review of a large PR can take 40 minutes or more, so keep the job timeout at 60. `INFO` findings appear in the body only and never block; everything `LOW` and above is posted inline and requests changes. Reviews do not require projects to have test coverage.

Every open review-bot thread must receive an explicit decision: `FIXED` or `NOT_APPLICABLE` resolves it (the latter with a posted reply), `OPEN` leaves it and blocks approval. The open thread IDs are baked into the verifier's output schema so it cannot skip one. If it does anyway, one retry classifies just the missed threads; anything still undecided after that is treated as `OPEN`. Threads are never resolved by omission. Inline comments carry the finding title, so a later round can tell it is re-raising the same finding. A re-raised finding is written as the invariant the code must satisfy rather than the next counter-example. A `MEDIUM` or `LOW` finding raised under the same title in two earlier rounds, where the author changed the commented code each time and the finding survived, is conceded: it moves to the dropped list with the invariant, and its open threads are resolved with that invariant as the reply, so a review cannot loop on one finding. A round the author ignored does not count, so an unaddressed finding keeps blocking. `HIGH` and `CRITICAL` findings are never conceded.

The reviewer's verdict cannot be argued down: a reply that accepts the risk instead of fixing or refuting the finding leaves the thread open, and the bot points at the waiver command instead of debating it. To accept a risk on record, reply `@review waive <reason>` in the thread. The bot skips Codex, posts `Waived by @login: <reason>`, and resolves that thread and every open thread raised under the same title. Only authors whose association is in `waive-roles` (default `OWNER`; set `MEMBER` for organisation-owned repositories) can waive, and a waiver without a reason is ignored. Only the bot's own marked reply counts as the record, so neither PR text nor a Codex reply can create one. While the waived thread is current, later rounds keep that exact title out of the review in every scope and list it under "Accepted risks" with the reason. Once the commented code changes, GitHub marks the thread outdated and the waiver lapses, so the finding can be raised again.
